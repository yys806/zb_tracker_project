...
\newcommand{\sharedimgfig}[3]{
  \begin{figure}[H]
    \centering
    \IfFileExists{../media/images/#1}{
      \includegraphics[width=#2\linewidth]{../media/images/#1}
    }{
      \fbox{\parbox{0.86\linewidth}{\centering\vspace{1.0cm}Image file: \code{../media/images/#1}\vspace{1.0cm}}}
    }
    \caption{#3}
    \vspace{-0.2em}
  \end{figure}
}
\newcommand{\sharedtriplefig}[7]{
  \begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{../media/images/#1}
      \caption{#2}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{../media/images/#3}
      \caption{#4}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{../media/images/#5}
      \caption{#6}
    \end{subfigure}
    \caption{#7}
    \vspace{-0.2em}
...
\MakeAbstract{
本报告记录了综合设计实践B课程大作业“基于 OrangePi 的两自由度云台目标跟踪系统”的设计、实现、调试与实验结果。系统以 OrangePi 5 Pro/RK3588S 开发板为主控平台，连接 USB 摄像头、PCA9685 PWM 舵机驱动板和两自由度云台，形成从视频采集、HSV 颜色目标检测、状态判断、云台控制到网页远程监看的闭环系统。主线算法采用 Python + OpenCV 完成颜色分割、形态学去噪、轮廓筛选和目标中心估计，再由带死区、限幅、滞回和平滑约束的双舵机控制器驱动云台，使高对比便利贴目标尽量保持在画面中心附近。网页控制台作为系统主入口，支持实时视频显示、自动中心标定、开始跟踪、急停、复位和状态查看。

在主线颜色跟踪之外，系统增加了基于 MediaPipe Hands 的手势识别扩展，能够提取 21 个手部关键点并绘制骨架连线；连续识别到 \code{one} 手势时，系统以食指指尖作为云台跟踪目标，连续识别到 \code{ok} 手势时退出指尖跟踪并执行安全复位。围绕课程加分项，项目实现了 C/C++ 视觉算子 wheel 封装并与 Python/OpenCV/pymp 版本进行性能对比，同时完成网页网络控制台、mock 云端上传和运动可视化。实测结果表明，系统在网页真实跟踪模式下平均帧率约 23 FPS；代表性运行中平均水平误差为 68.91 像素、平均垂直误差为 44.70 像素，丢失后平均重捕获时间约 0.76 秒。C++ wheel 模块能够正确编译与调用，输出与 OpenCV 一致。
}{OrangePi；两自由度云台；目标跟踪；OpenCV；PCA9685；网页控制台；MediaPipe Hands；手势识别；C++ wheel}

\MakeAbstractEng{
This report presents a two-axis pan-tilt target tracking system for the Comprehensive Design Practice B course project. The system uses an OrangePi 5 Pro/RK3588S board, a USB camera, a PCA9685 PWM servo driver and a two-degree-of-freedom gimbal. It implements a complete closed loop from video capture and HSV-based color target detection to bounded servo control and web-based remote monitoring. The web console provides live video, automatic center calibration, tracking control, emergency stop, reset and status visualization.

Beyond color-card tracking, the system integrates a gesture interaction module based on MediaPipe Hands. It extracts 21 hand landmarks, draws a skeletal hand overlay, starts fingertip tracking when a stable one-finger gesture is detected, and stops fingertip tracking with a safe reset when an OK gesture is detected. Bonus work includes a C/C++ wheel morphology operator, a Python/OpenCV/pymp benchmark, web networking, motion visualization and mock cloud upload. In real web tracking mode, the system reaches about 23 FPS; in a representative run, the average horizontal and vertical center errors are 68.91 px and 44.70 px, and the average reacquisition time is about 0.76 s.
}{OrangePi, Pan-tilt Control, Target Tracking, OpenCV, PCA9685, MediaPipe Hands, Gesture Recognition, C++ Wheel}
...
手势识别是当前版本新增的人机交互扩展。早期版本使用 YCrCb/HSV 肤色分割、形态学去噪、最大轮廓、凸包和凸缺陷估计手指数，但实测发现该方法对光照、背景肤色干扰和手掌姿态非常敏感，容易把桌面、手臂或阴影误认为手。为提高稳定性，最终版本优先接入 MediaPipe Hands 预训练手部关键点模型。程序启动时会根据配置尝试初始化 \code{mediapipe.solutions.hands.Hands}，当前设置为单手识别、视频流模式，并使用检测置信度和跟踪置信度筛选结果。模型成功工作时，系统从每帧图像中获得 21 个手部关键点，包括手腕、各手指掌指关节、近端关节、远端关节和指尖。网页叠加层不再绘制普通矩形框，而是按照 MediaPipe 的手部拓扑连接关键点，形成由点和线组成的手部骨架，更直观地展示各根手指的位置。

手势分类基于关键点几何关系实现。系统首先判断拇指、食指、中指、无名指和小指是否伸展，再结合拇指与食指、中指之间的距离判断 \code{ok}、\code{seven} 等特殊手势，常规数字手势则由伸展手指数映射到 \code{one}、\code{two}、\code{three}、\code{four}、\code{five} 等类别。为了减少单帧误识别造成的误触发，应用层不会直接相信单帧结果，而是要求同一手势连续稳定出现若干帧后才执行动作。

该扩展默认关闭，网页控制台提供独立开关。开启后，状态接口会返回手势类别、手指数、置信度、识别后端、中心点、关键点、连线和推理耗时等信息，Web 端可以直接看到后端是 \code{mediapipe} 还是 \code{traditional}。在 OrangePi 5 Pro 板端验证时，\code{mediapipe==0.10.18} 可以正常导入，探针输出 \code{mediapipe available: True}、\code{active backend: mediapipe}、\code{mp error: None}，说明当前手势识别已经由 MediaPipe 后端接管。若板端没有安装 MediaPipe 或初始化失败，系统会自动回退到传统轮廓方案，保证手势模块不会导致主程序不可用。

手势与云台控制之间采用独立模式切换。关闭该扩展时，HSV 便利贴跟踪和云台控制主流程不受影响；开启后，连续识别到 \code{one} 手势时，系统取 MediaPipe 第 8 号关键点，即食指指尖，构造一个虚拟目标点，并把该点送入原有云台控制闭环，使云台调整姿态让食指尖靠近画面中心。连续识别到 \code{ok} 手势时，系统退出指尖跟踪，停止云台跟踪状态，执行急停、释放 PWM、平滑复位和再次释放 PWM 的安全流程。该设计把“手势识别”和“舵机动作”之间隔了一层稳定帧判断与模式状态机，避免一般手势误识别直接干扰颜色目标跟踪主线。

\sharedimgfig{web_gesture_panel.png}{0.92}{网页控制台的手势识别扩展信息}
\sharedtriplefig{gesture_five_detection.png}{五指识别}{gesture_fingertip_tracking.png}{食指尖跟踪}{gesture_ok_reset.png}{OK 复位}{手势识别、指尖跟踪与复位过程截图}
...
在稳定跟踪主线之外，项目增加了手势识别、指尖跟踪和运动可视化扩展。手势识别最终采用 MediaPipe Hands 作为优先后端，通过预训练手部关键点模型提取 21 个手部 landmark，并在网页画面中绘制关键点骨架；再根据指尖、关节和手掌尺度之间的相对位置估计手指数和手势类别。当 MediaPipe 不可用时，系统自动回退到肤色分割、轮廓筛选、凸包和凸缺陷统计方案。Web 端可以独立开关该模块，并显示识别后端、手势类别、手指数和置信度等信息。在手势识别开启后，系统通过连续稳定帧过滤触发动作：稳定识别到 \code{one} 时，将食指尖关键点作为云台控制目标；稳定识别到 \code{ok} 时，停止指尖跟踪并执行安全复位。运动可视化部分则把光流箭头、轨迹回放、区域热力图、画面质量指标和事件日志整合到浏览器界面中，使系统不仅能“跟踪目标”，也能展示目标运动过程和画面状态。
...
项目改进方向包括四个方面：一是采集目标始终处于画面内的标准运行日志，进一步量化稳定跟踪能力；二是在视觉检测中加入颜色直方图或模板匹配辅助，提高对光照变化的鲁棒性；三是将 C++ 加速对象从单个形态学算子扩大到“阈值分割 + 形态学 + 候选框筛选”组合模块；四是继续优化手势关键点稳定性、指尖跟踪触发条件和分类边界，使手势扩展在不同光照、距离和手掌角度下保持更可靠的展示效果。
...
项目同时在报告素材目录中保留了本地视频文件，按功能整理在 \code{docs/报告/media/videos} 下，便于离线归档和课堂展示。视频文件说明如表~\ref{tab:local-videos} 所示。

\begin{table}[H]
  \centering
  \caption{本地演示视频文件}
  \label{tab:local-videos}
  \small
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{p{0.38\linewidth}p{0.50\linewidth}}
    ...
    文件名 & 内容说明 \\
    \midrule
    \code{demo_step03_simulation.mp4} & 网页模拟模式运行过程，不驱动真实舵机，用于验证视频流、检测框和控制接口。 \\
    \code{demo_step04_tracking.mp4} & 网页真实跟踪模式运行过程，展示便利贴目标与云台闭环跟随效果。 \\
    \code{demo_modules_extended.mp4} & 增加扩展模块后的综合演示，包含网页控制台、运动可视化和交互功能。 \\
    \code{demo_final_defense.mp4} & 最终答辩演示视频，用于展示完整系统运行效果。 \\
    \bottomrule
  \end{tabular}
\end{table}
